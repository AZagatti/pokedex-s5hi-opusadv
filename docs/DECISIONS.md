...
## Accessibility fixes found by running audits against real rendered pages

**What:** `--accent-text-bg` (a `color-mix(in srgb, var(--accent), black 10%)` darkened variant of `--accent`) now backs every small-text-on-accent element found across the app: the sprite-switcher's active tab, the "hidden ability" tag, the evolution chain's "current" item, and the 404/empty-favorites "back" links. The same pattern also fixed the list page's active type-filter chips (previously hardcoded `color: #fff`). The evolution chain's current-item level text (`<small>`, normally `opacity: 0.75` for de-emphasis against the neutral surface background) is kept at full opacity specifically when it's sitting on the accent background, since diluting an already-corrected contrast ratio undoes the fix. Separately, the cry button's `aria-label` changed from `` `Play ${name}'s cry` `` to `` `Play cry for ${name}` ``, and the 404 page gained a `<meta name="description">` it was missing entirely.

**Why:** White text on the light theme's `--accent` (`#e63946`) measures 4.24:1 contrast — just under WCAG AA's 4.5:1 for small text. Flagged by Lighthouse's `color-contrast` audit on the detail page; a second pass on the same page found a third instance (the evolution chain badge) after the first two were fixed, and the same hardcoded pattern was then proactively fixed everywhere else it appeared rather than waiting to audit every single page individually. Separately, `label-content-name-mismatch` flagged the cry button: its visible text ("Play cry") wasn't a substring of its accessible name ("Play charizard's cry"), which breaks voice-control navigation ("click play cry" wouldn't match). The missing meta description was a plain `meta-description` SEO audit failure on the 404 page specifically (every other page already had one). All of this was caught by actually running Lighthouse against real rendered pages — `/`, `/berries`, `/pokemon/[name]` (via `mode: snapshot`), `/favorites`, and the 404 page all score 100/100/100/100 where the metric is measurable, not by inspection alone.

**A caching trap worth flagging:** the meta-description fix appeared to have zero effect on the built output — same compiled JS, same missing tag — across several `rm -rf .svelte-kit build && npm run build` cycles. The actual cause was `node_modules/.vite`'s own transform cache serving a stale compiled `+error.svelte` chunk; `rm -rf node_modules/.vite` (in addition to `.svelte-kit`/`build`) was required before the change would actually appear. Worth remembering if a source edit ever seems to have no effect on a rebuild despite a clean `.svelte-kit`/`build`.
